...
interface SaveStatsModalProps {
  clubName: string;
  existingSeasons: StatsSeason[];
  onSave: (name: string, club: string) => Promise<void>;
  onClose: () => void;
}

export default function SaveStatsModal({ clubName, existingSeasons, onSave, onClose }: SaveStatsModalProps) {
  ...
  const duplicate = isValid ? existingSeasons.find((s) => s.name === fullName) : undefined;

  const handleSave = async () => {
    if (!isValid) return;
    if (duplicate && !confirming) { setConfirming(true); return; }
    setLoading(true);
    try {
        ...
        {saved ? (
          ...
          <>
            ...
            {confirming && duplicate && (
              <div className="rounded-lg border px-3 py-2.5 text-xs space-y-1" style={{ borderColor: "#f9731640", background: "#f9731610" }}>
                <p className="font-bold" style={{ color: "#f97316" }}>Ya existen estadísticas con ese nombre</p>
            ...
            {!confirming && duplicate && isValid && (
              <p className="text-[10px]" style={{ fontFamily: "var(--font-mono)", color: "#f97316" }}>
                Ya existen estadísticas guardadas como <span className="font-bold">{fullName}</span>
              </p>
            )}
            ...
            <div className="flex gap-2 pt-1">
              <button
                ...
                style={{ borderColor: "var(--color-border-subtle)", color: "var(--color-text-muted)", background: "transparent" }}
              >
